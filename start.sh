...
echo "[START] StreamLens 明鉴 -> http://127.0.0.1:5000/web"
PYTHON_BIN="python3"
[ -x .venv/bin/python ] && PYTHON_BIN=".venv/bin/python"
(nohup "$PYTHON_BIN" src/main.py -m http -p 5000 > server.log 2>&1 &)
sleep 10
...
echo "Server running in background. Log: server.log. Stop: pkill -f 'src/main.py -m http'"
